...
function preload() {
  relayFont = loadFont("https://fonts.cdnfonts.com/s/94295/RelayFun.woff");
  seedFallingImg = loadImage("https://i.postimg.cc/VNcXp2DH/elements-02.png");
  seedSplashImg  = loadImage("https://i.postimg.cc/9ftyh1kX/elements-03.png");
} 
...
function draw() {
  ...
  if (state === "falling") {
    imageMode(CENTER);
    image(seedFallingImg, seedX, seedY);
    ...
    imageMode(CENTER);
    image(seedSplashImg, width / 2, height - 120);

  } else if (state === "showResult") {
    if (selectedAsset) {
      imageMode(CENTER);
      image(selectedAsset, width / 2, height - 80 - 488 / 2, 700, 488);
    }
    ...
    if (selectedQuote) {
      ...
      let x = width / 2;
      let y = height - 488 - 80 - 50; // quote cách top hình 190px
      let boxWidth = width * 0.8;
      text(selectedQuote, x, y, boxWidth, 400);
    }
  }
}
...
function chooseAssetAndQuote() {
  selectedAsset = random(assetsSeeds);
